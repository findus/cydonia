...
public class ServerConfigFrame extends JFrame implements KeyListener,
		ActionListener, ServerStateListener, Console {

	protected static final String MAPFOLDER = "/de/encala/cydonia/level/";

	private static final FileFilter mfxFilter = new FileFilter() {
		@Override
		public boolean accept(File pathname) {
			if (pathname.isFile()
					&& pathname.getName().endsWith(GameServer.MAPEXTENSION)) {
				return true;
			}
			return false;
		}
	...
	private static final javax.swing.filechooser.FileFilter mfxChooserFilter = new FileNameExtensionFilter(
			"XML Map Files", "mfx");
	...
	public ServerConfigFrame(GameServer server) {
		...
		try {
			BufferedImage favicon = ImageIO.read(ServerConfigFrame.class.getResourceAsStream("/de/encala/cydonia/gui/favicon.png"));
	...
	private String[] getDefaultMapNames() {
		InputStream is = this.getClass().getResourceAsStream(
				"/de/encala/cydonia/level/levels.txt");
	...
	private void loadMapsFromDir() {
		...
		if (userdir.exists() && userdir.isDirectory()) {
			...
			if (file.exists() && file.isDirectory()) {
				...
				for (File m : maps) {
					listmodel.addElement(m.getName().substring(0,
							m.getName().indexOf(GameServer.MAPEXTENSION)));
				}
			}
		}
	}
	...
	private void saveMap() {
		String mapname = JOptionPane.showInputDialog(
				"Please insert a name for this map:", server
						.getWorldController().getMap().getName());
			...
			if (JFileChooser.APPROVE_OPTION == chooser.showSaveDialog(this)) {
				...
				if (!f.exists()) {
					accepted = true;
						...
						.showConfirmDialog(
								this,
								"File already exits. Do you want to overwrite it?",
	...
	public void stateChanged() {
		if (server.getWorldController() != null
				&& server.getWorldController().getMap() != null) {
