...
public class ServerPicker extends AbstractServerEquipment {
	...
	public EquipmentInfo getInfo() {
		List<Long> repo = new LinkedList<Long>();
		for(ServerFlube f : this.repository) {
			repo.add(f.getId());
		}
		return new PickerInfo(this.name, this.range, this.capacity, repo, this.player.getId(), TYPENAME);
	}
