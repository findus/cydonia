...
public class ServerPusher extends Pusher {
	 ...
	 */
	public ServerPusher(String name, float force, long interval, Player player, MainController mainController) {
		...
		if(this.lastShotTime + this.getInterval() < System.currentTimeMillis()) {
			...
			if(result != null && result.getGeometry().getParent() != null && result.getGeometry().getParent().getName() != null && result.getGeometry().getParent().getName().startsWith("player")) {
				...
				if(victim != null && victim.getId() != this.player.getId()) {
					Vector3f forceVector = this.player.getViewDir().normalize().mult(this.getForce());
					PushEvent event = new PushEvent(this.getPlayer().getId(), victim.getId(), forceVector, true);
