...
public class ServerSwapper extends Swapper {
	...
	public void usePrimary(boolean activate) {
		...
		CollisionResults results = getMainController().getWorldController()
				.pickRootList(
		...
		for (CollisionResult result : results) {
			if (result != null && result.getDistance() <= this.getRange()) {
				if ((result.getGeometry().getParent() != null
						...
						.startsWith("player"))) {
					Player target = getMainController()
							.getPlayerController()
							.getPlayer(
					...
					break;
						...
						result.getGeometry())) {
					Flube target = getMainController().getWorldController()
							.getFlube(
									(long) result.getGeometry().getUserData(
											"id"));
					if (getMainController().getWorldController()
