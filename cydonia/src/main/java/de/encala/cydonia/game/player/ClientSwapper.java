...
public class ClientSwapper extends Swapper {
	...
	public void usePrimary(boolean activate) {
		...
		if (result != null && result.getDistance() <= this.getRange()) {
			if ((result.getGeometry().getParent() != null
					...
					.getGeometry().getParent().getName().startsWith("player"))) {
				Player target = getMainController().getPlayerController()
						.getPlayer(
								Integer.valueOf(result.getGeometry()
										.getParent().getName().substring(6)));
				markA(target);
			} else if (getMainController().getWorldController().isFlube(
					result.getGeometry())) {
				Flube target = getMainController()
						.getWorldController()
						.getFlube((long) result.getGeometry().getUserData("id"));
				markA(target);
			}
		}
	}
	...
	public void useSecondary(boolean activate) {
		...
		CollisionResult result = getMainController().getWorldController()
				.pickRoot(
						this.player.getEyePosition().add(
								player.getViewDir().normalize().mult(0.4f)),
						this.player.getViewDir());
		if (result != null && result.getDistance() <= this.getRange()) {
			if ((result.getGeometry().getParent() != null
					&& result.getGeometry().getParent().getName() != null && result
