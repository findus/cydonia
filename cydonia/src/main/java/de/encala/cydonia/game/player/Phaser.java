...
public class Phaser extends AbstractEquipment {
	 ...
	 */
	public Phaser(String name, float damage, long interval, Player player,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.encala.cydonia.game.player.Equipment#getTypeName()
	 */
	@Override
	...
	public void initGeometry() {
		...
		beam.setShape(new EmitterBoxShape(Vector3f.ZERO, new Vector3f(0.1f,
				0.1f, 0.1f)));
	...
	private void initSound() {
		fireSound = new AudioNode(getMainController().getAssetManager(),
				"de/encala/cydonia/sounds/pew01_mono.wav", false);
		...
		try {
			if (hudImg == null) {
				hudImg = ImageIO.read(this.getClass().getResourceAsStream(
						"/de/encala/cydonia/gui/hud/phaser.png"));
			}
		} catch (IOException e) {
		}
	}
	...
	public void usePrimary(boolean activate) {
		...
		if (this.lastShotTime + this.getInterval() < System.currentTimeMillis()) {
			...
			if (player.getViewDir().normalize().getY() > 0)
				angle = -angle;
			geom.setLocalRotation(new Quaternion().fromAngleAxis(angle,
					Vector3f.UNIT_X));

			beam.setLocalTranslation(0, 0, distance / 2);
			beam.setShape(new EmitterBoxShape(new Vector3f(-0.01f, -0.01f,
					-distance / 2), new Vector3f(0.01f, 0.01f, distance / 2)));
