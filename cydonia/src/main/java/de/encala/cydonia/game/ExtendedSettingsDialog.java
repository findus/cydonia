...
public class ExtendedSettingsDialog extends JDialog {
	...
	public static final int NO_SELECTION = 0, APPROVE_SELECTION = 1,
			CANCEL_SELECTION = 2;
	...
	private String[] windowedResolutions = { "320 x 240", "640 x 480",
	 ...
	 */
	public ExtendedSettingsDialog(AppSettings source, String imageFile,
	 ...
	 *             if the source is <code>null</code>
	 */
	public ExtendedSettingsDialog(AppSettings source, URL imageFile,
			boolean loadSettings) {
		if (source == null) {
	...
	private void createUI() {
		...
		if (source.getIcons() != null) {
			safeSetIconImages((List<BufferedImage>) Arrays
					.asList((BufferedImage[]) source.getIcons()));
		}
	...
	private void safeSetIconImages(List<? extends Image> icons) {
		try {
			...
			if (owner != null) {
				Method setIconImages = owner.getClass().getMethod(
						"setIconImages", List.class);
				setIconImages.invoke(owner, icons);
				return;
			}

			Method setIconImages = getClass().getMethod("setIconImages",
					List.class);
	...
	private boolean verifyAndSaveCurrentSelection() {
		...
		String renderer = "LWJGL-OpenGL2";// (String)
	...
	private JComboBox setUpRendererChooser() {
		String[] modes = { "NULL", "JOGL-OpenGL1", "LWJGL-OpenGL2",
	...
	private void updateAntialiasChoices() {
		...
		// through pbuffer
		String[] choices = new String[] { "Disabled", "2x", "4x", "6x", "8x",
				"16x" };
		antialiasCombo.setModel(new DefaultComboBoxModel(choices));
		antialiasCombo.setSelectedItem(choices[Math.min(
				source.getSamples() / 2, 5)]);
	}
	...
	private void updateShadowChoices() {
		...
		shadowCombo
				.setSelectedItem(choices[source.get("shadowLevel") != null ? (Integer) source
						.get("shadowLevel") : 0]);
	}
	...
	/**
	 * Returns every unique resolution from an array of <code>DisplayMode</code>
	 * s.
	 */
