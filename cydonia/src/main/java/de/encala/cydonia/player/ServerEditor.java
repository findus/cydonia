...
public class ServerEditor extends Editor {
	...
	public void usePrimary(boolean activate) {
		...
		if (result != null
				...
				&& getMainController().getWorldController().isFlube(
						result.getGeometry())) {
			...
			if ("flube".equalsIgnoreCase(getObjectType())) {
				...
				id = getMainController().getWorldController()
						.getFreeSpawnPointId();
			}
	...
	public void useSecondary(boolean activate) {
		...
		if (result != null && result.getDistance() <= this.getRange()) {
			if (getMainController().getWorldController().isFlube(
					result.getGeometry())) {
			...
			} else if (getMainController().getWorldController().isFlag(
					result.getGeometry())) {
				Flag f = getMainController().getWorldController().getFlag(
						(Integer) result.getGeometry().getUserData("id"));
				RemoveEvent remove = new RemoveEvent(this.player.getId(),
						f.getId(), "flag", true);
				getMainController().getEventMachine().fireEvent(remove);
			} else if (getMainController().getWorldController().isSpawnPoint(
					result.getGeometry())) {
				SpawnPoint sp = getMainController().getWorldController()
						.getSpawnPoint(
